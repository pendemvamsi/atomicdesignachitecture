...
export function Sidebar({ activeSection, onSectionChange, isCollapsed, onToggle }: SidebarProps) {
  return (
    <motion.aside
      initial={{ width: 240 }}
      animate={{ width: isCollapsed ? 72 : 240 }}
      transition={{ duration: 0.3, ease: "easeInOut" }}
      className="bg-slate-900 border-r border-slate-800 h-screen sticky top-0 flex flex-col"
    >
      <div className="p-4 border-b border-slate-800 flex items-center justify-between">
        {!isCollapsed && (
          ...
          >
            ...
            <span className="text-white font-bold">Atomic Design</span>
          </motion.div>
        )}
        <button
          onClick={onToggle}
              ...
              {!isCollapsed && (
                ...
                >
                  ...
                  <span className={`text-xs px-1.5 py-0.5 rounded-md ${
                    isActive ? `bg-${section.color}-500/30` : "bg-slate-800"
                  }`}>
                    {section.count}
                  </span>
                </motion.div>
      ...
      {!isCollapsed && (
        ...
        >
          <div className="bg-gradient-to-r from-rose-500/10 via-amber-500/10 to-emerald-500/10 rounded-xl p-3 border border-slate-700">
            <p className="text-slate-300 text-xs font-medium mb-1">Total Components</p>
            <p className="text-white text-2xl font-bold">340+</p>
            <p className="text-slate-500 text-xs mt-1">5 layers of abstraction</p>
          </div>
        </motion.div>
      )}
    </motion.aside>
